...
def dump_all(
    ...
    # Output directory mirrors the asset name path
    out_dir = output_root / asset_name
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    for idx, obj in enumerate(env.objects):
        ...
        if exporter:
            ...
            # Generic: try typetree dump
            files = _export_typetree(obj, out_dir, idx, f"{type_name.lower()}_{idx:04d}")

        if files:
            stats[type_name] = stats.get(type_name, 0) + len(files)

    # Remove output dir if nothing was exported (keep tree clean)
    if not stats:
        with contextlib.suppress(OSError):
            out_dir.rmdir()
...
def dump_all(
    ...
    for i, entry in enumerate(entries):
        ...
        # Skip if output directory already has content
        out_dir = output_root / asset_name
        if skip_existing and out_dir.is_dir() and any(out_dir.iterdir()):
            skipped += 1
            processed += 1
        ...
        try:
            stats = dump_bundle(
                file_path, entry["key"], asset_name, output_root, type_filter
            )
            for k, v in stats.items():
                agg_stats[k] = agg_stats.get(k, 0) + v
        except Exception as e:
            errors += 1
            log.debug("Error processing %s: %s", asset_name, e)
