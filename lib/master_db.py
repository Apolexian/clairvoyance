...
CATEGORY_SUPPORT_CARD_NAME_FULL = (
    75  # support card full names "[Title] CharaName" via support_card_data.id
)
CATEGORY_SUPPORT_CARD_TITLE = 76  # support card titles (just "[Title]") via support_card_data.id
...
def support_card_name(sc_id: int | str | None) -> str:
    """Return the support card name (text_data category 75 '[Title] Name', or 76 '[Title]')."""
    if sc_id is None:
        return "?"
    try:
        sid = int(sc_id)
    ...
    # Category 75: full name like "[Title] CharaName"
    names = _load_text_data(CATEGORY_SUPPORT_CARD_NAME_FULL)
    name = names.get(sid)
    if name:
        return name
    # Category 76: just the title bracket
    names76 = _load_text_data(CATEGORY_SUPPORT_CARD_TITLE)
    return names76.get(sid, str(sid))
...
def scenario_name(scenario_id: int | str | None) -> str:
    """Return the scenario name (text_data category 119, e.g. 'URA Finale')."""
    ...
    names = _load_text_data(CATEGORY_SM_SCENARIO_NAME)
...
@lru_cache(maxsize=1)
def _load_story_choices() -> dict[int, list[str]]:
    """
    Load extracted choice texts from story_choices.json.

    This file is produced by extract_story_text.py which reads game asset bundles
    to extract the actual choice text labels shown to the player.

    Returns {story_id: ["Choice 1 text", "Choice 2 text", ...]}.
    """
    choices_file = _APP_DIR / "story_choices.json"
    if not choices_file.is_file():
        return {}
    try:
        raw = json.loads(choices_file.read_text(encoding="utf-8"))
        if not isinstance(raw, dict):
            return {}
        # Keys in JSON are strings; convert to int
        return {int(k): v for k, v in raw.items() if isinstance(v, list)}
    except Exception as e:
        log.warning("Failed to load story_choices.json: %s", e)
        return {}


def story_choice_texts(story_id: int | str | None) -> list[str] | None:
    """
    Return the choice option texts for a story event, or None if not available.

    Requires story_choices.json to have been generated by extract_story_text.py.
    """
    if story_id is None:
        return None
    try:
    ...
    choices = _load_story_choices()
    return choices.get(sid)
...
def event_info(story_id: int | str | None) -> dict | None:
    ...
    # Look up choice text labels from extracted story data
    choice_texts = story_choice_texts(sid)
    if choice_texts:
        info["choice_texts"] = choice_texts

    return info


def event_id_to_story_id(event_id: int | str | None) -> int | None:
    """
    Resolve a runtime event_id to its story_id via single_mode_story_data.

    The event_id from CheckEvent requests / unchecked_event_array corresponds
    to single_mode_story_data.id.  Returns the story_id, or None.
    """
    if event_id is None:
        return None
    try:
    ...
    # _load_story_data is keyed by story_id — use reverse index for id→story_id lookup
    rev = _event_id_reverse_index()
    return rev.get(eid)


@lru_cache(maxsize=1)
def _event_id_reverse_index() -> dict[int, int]:
    """Build {story_data_id → story_id} reverse index."""
    story_data = _load_story_data()
    rev: dict[int, int] = {}
    for story_id, sd in story_data.items():
        sdid = sd.get("story_data_id")
        if sdid:
            rev[sdid] = story_id
    return rev


def event_name_by_event_id(event_id: int | str | None) -> str:
    """Resolve event_id → story_id → human-readable event name."""
    sid = event_id_to_story_id(event_id)
    if sid:
        return story_name(sid)
...
def _clear_caches() -> None:
    ...
    _event_id_reverse_index.cache_clear()
...
def annotate_ids(obj: object, _path: str = "") -> dict[str, str]:
    ...
    # Keys that need a custom lookup (not a simple text_data category)
    _CUSTOM_KEYS = {"event_id"}
    ...
    if isinstance(obj, dict):
        for k, v in obj.items():
            ...
            if isinstance(v, (int, float)) and v != 0:
                ...
                # Standard text_data lookups
                if k in _ANNOTATABLE_KEYS:
                ...
                # Custom lookups (event_id → story_id → name)
                elif k in _CUSTOM_KEYS:
                    if k == "event_id":
                        resolved = event_name_by_event_id(vid)
                        if resolved == str(vid):
                            resolved = None  # no point annotating with itself
